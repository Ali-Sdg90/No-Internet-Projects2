...
const CreateBasePage = () => {
    ...
    const createBase = () => {
                ...
                .forEach((_) => {
                    ...
                    while (!isShipSeedOk && retries++ < maxRetries) {
                        ...
                        for (
                            ...
                            if (checkNeighbors(current_i, current_j, newBase)) {
                                // console.log(
                                //     "BAD SEED >>",
                                //     current_i,
                                //     current_j
                    ...
                    for (let shipTile = 0; shipTile < sizeOfShip; shipTile++) {
                        let current_i = shipSeed_i;
                        let current_j = shipSeed_j + shipTile;
                        if (shipSeed_dir) {
                            current_i = shipSeed_i + shipTile;
                        ...
                        newBase = addWaterShadow(newBase, current_i, current_j);
        ...
        setBaseArrays((prev) => [...prev, newBase]);
    };

    const createNewBases = () => {
        setBaseArrays([]);

        Array(NUMBER_OF_BASES)
            .fill()
            .forEach(() => createBase());

        setShowBaseIndex(Math.floor(NUMBER_OF_BASES / 2));
    };

    useEffect(() => {
        // Create bases and push them to the baseArrays
        createNewBases();
        // console.log("===>", baseArrays);
    }, []);

    const confirmBase = () => {
        // console.log("Confirmed Base >>", baseArrays[showBaseIndex]);
        setUserGround(baseArrays[showBaseIndex]);
        setGameLocation("GamePage");
    };
        ...
        <div className="page-elements">
            {baseArrays?.length > 0 ? (
                <>
                    ...
                    <BaseNavigator
                        showBaseIndex={showBaseIndex}
                        setShowBaseIndex={setShowBaseIndex}
                        baseArrays={baseArrays}
                    />

                    <div className="component-btns">
                        <div
                            className="create-new-bases"
                            onClick={createNewBases}
                        >
                            Create New Bases
                        </div>
                        <div className="confirm-base" onClick={confirmBase}>
                            Confirm Base
                        </div>
                    </div>
                </>
